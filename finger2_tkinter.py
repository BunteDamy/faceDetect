...
window = tk.Tk()
window.title("Video İşleme Arayüzü")
...
def start_camera():
    ...
    def detect_finger(contour):
        ...
        if defects is None:
            return 0
        finger_count = 0
        for i in range(defects.shape[0]): # shape = shape[0], ilgili veri yapısının ilk boyutunu, yani satır sayısını belirtir.
            s, e, f, d = defects[i, 0] 
            start = tuple(contour[s][0]) # parmak başlangıç
            end = tuple(contour[e][0]) # parmak bitiş
            far = tuple(contour[f][0]) # derin nokta
            a = np.linalg.norm(np.array(start) - np.array(end)) # linalg.norm -> bir vektör ya da matrisin uzunluğunu (norm) hesaplamak için. 2 nokta arasındaki düz mesafe. array olarak almak lazım.
    ...
    while True:
        ...
        grey = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) # kameradaki görüntüyü grileştirir. Çünkü bilgisayar daha rahat okur
        ...
        for (x, y, w, z) in faces:
            ...
            lower_skin = np.array([0, 20, 70], dtype=np.uint8) # cilt alt tonu
            upper_skin = np.array([20, 255, 255], dtype=np.uint8) # cilt üst tonu
            ...
            if contours:
                ...
                finger_count = detect_finger(max_contour) # parmak sayısı için fonksiyona git
